			<footer>
				<div class="latim">
					Ignorantia <br />
					Legis Neminem<br />
					Excusat
				</div>

				<div class="assine-agora">
					<div class="assine-agora">Assine agora</div>
					<div class="orientador">Orientador Empresarial</div>
					<div class="cinco-zero">50<small>%<span>off</span></small></div>

					<div class="categorias">
						Previdência Social<br />
						Segurança e Saúde no Trabalho<br />
						Trabalho
					</div>
				</div>

				<div class="footer-links">
					<div class="links">
						<a href="javascript: void(0)">Acesse a nossa página</a><br />
						<a href="javascript: void(0)">Esqueceu login e senha</a><br />
						<a href="javascript: void(0)">Fale Conosco</a>
					</div>

					<div class="social">
						<a href="javascript: void(0)">
							<img src="http://dl.dnzdns.com/v/5h6k73ABF0285" alt="" /><!-- Facebook -->
						</a>
						<a href="javascript: void(0)">
							<img src="http://dl.dnzdns.com/v/QfBNafABF0214" alt="" /><!-- Google Plus -->
						</a>
						<a href="javascript: void(0)">
							<img src="http://dl.dnzdns.com/v/fwO604ABF0228" alt="" /><!-- LinkedIn -->
						</a>
						<a href="javascript: void(0)">
							<img src="http://dl.dnzdns.com/v/tTtM6bABF0284" alt="" /><!-- Twitter -->
						</a>
					</div>
				</div>

				<div class="equipe-tecnica">
					<div class="title">
						EQUIPE VERITAE
					</div>

					<div class="title-underline"></div>

					<div class="equipe-tecnica-lista">
						<strong>Conteúdo e Consultoria</strong><br />
						Adenisio Pereira da Silva Junior<br />
						Beatris Papandreu<br />
						Sofia Kaczurowski<br />
						Luis Eduardo Yamagata<br />
						veritae@veritae.com.br<br />
						consultoria@veritae.com.br<br />
						Fones: 55 (21) 25240487 | 34714457<br />

						<strong>Formatação e Diagramação</strong><br />
						Cândida Duarte<br />
						adm@veritae.com.br<br />
						Fones: 55 (21) 25240487 | 34714457	<br />

						<strong>Assinaturas - Relacionamento - Treinamentos</strong><br />
						Beatris Papandreu<br />
						Elisabeth Campos<br />
						Francinne Hellora<br />
						Hélio K.Ka.Yamagata<br />
						Kathleen Yamágatá<br />
						veritae@veritae.com.br<br />
						cursos@veritae.com.br<br />
						Fones: 55 (21) 41415039 | 25240487<br />

						<strong>Tecnologia e Inovação</strong><br />
						Danilo C. França<br />
						Frank Mattos<br />
						Hélio K.Ka. Yamagata<br />
						Julian Andrade<br />
						adm@veritae.com.br<br />
						Fones: 55 (21) 25240487 | 34714457<br />

						<strong>Produção</strong><br />
						VERITAE Orientador Empresarial Ltda<br />
						veritae@veritae.com.br<br />
						Fones: 55 (21) 41415039 | 25240487 | 34714457<br />

						<strong>Administrativo - Financeiro - Fiscal</strong><br />
						Janaína T.<br />
						Tânia Mendes<br />
						Telles e Mello<br />
						financeiro@veritae.com.br<br />
						adm@veritae.com.bra<br />
						Fones: 55 (21) 22159878 | 25240487 | 34714457 | 41415039<br />

						<strong>Editorial, Recebimento de Artigos, Sugestões de Publicações</strong><br />
						veritae@veritae.com.br<br />
						Fones: 55 (21) 41415039 | 25240487 | 34714457<br />

						<strong>Direção</strong><br />
						Sofia Kaczurowski<br />
						sofia@veritae.com.br<br />
						veritae@veritae.com.br<br />
					</div>
				</div>

				<div class="copyright">
					&copy; Copyright <?php echo date('Y') ?> VERITAE - All Rights Reserved.
				</div>

			</footer>

		</div> <!-- /#main -->
	</body>
</html>